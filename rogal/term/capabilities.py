...
List of predefined and user defined capabilities.

See: http://manpages.ubuntu.com/manpages/xenial/man5/terminfo.5.html
See: http://manpages.ubuntu.com/manpages/impish/en/man5/user_caps.5.html
...
FLAG_CAPABILITIES = set()
NUM_CAPABILITIES = set()
STR_CAPABILITIES = set()

_TERMINFO = None


PREDEFINED_FLAG_CAPABILITIES = AttrDict(
...
FLAG_CAPABILITIES.update(PREDEFINED_FLAG_CAPABILITIES.values())
for long_name, cap_name in PREDEFINED_FLAG_CAPABILITIES.items():
    Capability[long_name] = cap_name
    Capability[cap_name] = cap_name


PREDEFINED_NUM_CAPABILITIES = AttrDict(
...
NUM_CAPABILITIES.update(PREDEFINED_NUM_CAPABILITIES.values())
for long_name, cap_name in PREDEFINED_NUM_CAPABILITIES.items():
    Capability[long_name] = cap_name
    Capability[cap_name] = cap_name


PREDEFINED_STR_CAPABILITIES = AttrDict(
...
STR_CAPABILITIES.update(PREDEFINED_STR_CAPABILITIES.values())
for long_name, cap_name in PREDEFINED_STR_CAPABILITIES.items():
    Capability[long_name] = cap_name
    # Capability[long_name.upper()] = cap_name
    Capability[cap_name] = cap_name
...
# From user_caps
    ...
    'RGB',
    'U8',
    'XM',       # enables/disables xterm mouse mode
    'xm',       # format of the mouse responses
...
STR_CAPABILITIES.update(USER_DEFINED_CAPABILITIES[4:])
...
for cap_name in EXTENDED_KEY_DEFINITIONS:
    Capability[cap_name] = cap_name
    for modifier_num in range(2,9):
        cap_mod_name = f'{cap_name}{modifier_num}'
        Capability[cap_mod_name] = cap_mod_name
